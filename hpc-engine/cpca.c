...
int main( int argc , char ** argv ) 
{
    ...
    size_t  workarea_size=(size_t)mm_get_size_of_total_workspace(NXY); // minimal size of workarea
    int mpipackagesize;
...
//     if(ip==0 && md.spinsymmetry==1) print_warning(WSLDA_WRN_SPINSYMMETRY1);
    ...
    double *d_qf_densities=d_densities + 12*NXY; // to simplify notation
    
    // potentials
    gpu_exec( host_malloc_pl((size_t)12*NXY*sizeof(double), (void **)&h_potentials) );
    gpu_exec(     gpu_malloc((size_t)12*NXY*sizeof(double), (void **)&d_potentials) );   
...
//         wprintf("# WF SCATTER: ip=%d processes nwfip=%d wave-functions\n", ip, nwfip);
    ...
    else if(md.inittype==1) // Start from solution of st-wslda-1d solver
    {
        ...
        for(iwf=0; iwf<nwfip; iwf++)
        {
            ...
            for ( ix = 0 ; ix < NX ; ix++ ) for ( iy = 0 ; iy < NY ; iy++ )
            {
                Nmya+=(pow(creal(h_wavefun[          iwf*NXY+ixyz]),2)+pow(cimag(h_wavefun[          iwf*NXY+ixyz]),2))*fbeta(h_fbetaEn[iwf],beta)*wcnt;
                Nmyb+=(pow(creal(h_wavefun[nwfip*NXY+iwf*NXY+ixyz]),2)+pow(cimag(h_wavefun[nwfip*NXY+iwf*NXY+ixyz]),2))*(1.0-fbeta(h_fbetaEn[iwf],beta))*wcnt;
                ixyz++;
            }  
        }
    ...
    else if(md.inittype==2) // Start from solution of st-wslda-2d solver 
    {
        ...
        time=0.0;
        file_operation( check_comptibility(md.inprefix, 2, file_suffix) );
        sprintf(file_name, "%s/%s.info", md.inprefix, file_suffix);
        if(ip==0)
        {
            file_operation( read_checkpoint_info_pca(file_name, &nwf, &_nx, &_ny, &_nz, &_dx, &_dy, &_dz, &kF, &mu[0], &ec, &beta) );
    ...
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_wf) );
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_fkm1) );
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_fkm2) );
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_fkm3) );
#if INTEGRATION_SCHEME>=AB4AM5
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_fkm4) );
#endif
#if INTEGRATION_SCHEME>=AB5AM5
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_fkm5) );
#endif
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_wf_d_dx) );
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_wf_d_dy) );
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_wf_laplace) );
#ifndef FAST_CONST_EFFECTIVE_MASS_MODE
    gpu_exec( gpu_malloc(NXY*nwfip*2*sizeof(double complex), (void **)&d_alphawf_laplace) );
#endif
    ...
    { 
        // copy wave-functions
        gpu_exec( memcopy_host2gpu(h_wavefun, d_wf,  (size_t)2*nwfip*NXY*sizeof(double complex)) );   

        // we start from eigenstates - then fkm1, fkm2, fkm3 are zero
        for(i=0; i<NXY*nwfip*2; i++) h_wavefun[i]=0.0 + I*0.0;
        gpu_exec( memcopy_host2gpu(h_wavefun, d_fkm1,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_wavefun, d_fkm2,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_wavefun, d_fkm3,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
#if INTEGRATION_SCHEME>=AB4AM5
        gpu_exec( memcopy_host2gpu(h_wavefun, d_fkm4,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
#endif
#if INTEGRATION_SCHEME>=AB5AM5
        gpu_exec( memcopy_host2gpu(h_wavefun, d_fkm5,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
#endif

        // copy potentials
        gpu_exec( memcopy_host2gpu(h_potentials, d_potentials,  (size_t)12*NXY*sizeof(double)) );  
    }
    
    // copy weights
    // convert quasiparticle energies into weights, h_qpe_nwfip as temporary buffer
    ...
    if(workarea_size<sizeof(double)*nwfip*NXY) workarea_size=sizeof(double)*nwfip*NXY; // workspace needed by reduce_many(...)
...
#endif
    // potentials
//     if(md.inittype!=5) gpu_exec( compute_potentials(it, d_densities, d_potentials, 0.0, md.nthreads) );
    // energy
    ...
    wdata_metadata wdmd; 
    file_operation( create_wdata_metadata(&md, 2, t0, md.timesteps*dt, md.spinsymmetry, &wdmd) );
    ...
    if(ip==0) 
    {
        ...
        file_operation( write_wdata_metadata_file(&md, &wdmd, "td-wslda-2d") );
    }
    MPI_Barrier(MPI_COMM_WORLD);
    
    gpu_exec( memcopy_gpu2host(d_densities, h_densities,  (size_t)12*NXY*sizeof(double)) );
    gpu_exec( memcopy_gpu2host(d_potentials, h_potentials,  (size_t)12*NXY*sizeof(double)) );
    ...
    {
        // densities - local reduction
        gpu_exec( calculate_densities(nwfip, d_wf, d_wf_d_dx, d_wf_d_dy, d_wf_laplace, d_kkz, d_fbetaEn, d_weight_subset, d_densities_subset, gradients_computed, md.nthreads) );
        // densities - global reduction
        gpu_exec( memcopy_gpu2host(d_densities_subset, h_densities_subset,  (size_t)12*NXY*sizeof(double)) ); 
        MPI_Allreduce( MPI_IN_PLACE, h_densities_subset, 12*NXY, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    { 
        ...
        for(i_step=0; i_step<selfstart_steps; i_step++)
        {
            ...
            gpu_exec( memcopy_gpu2gpu(d_wf, d_fkm3, (size_t)2*nwfip*NXY*sizeof(double complex)) );
            ...
            // Store H*Psi
            gpu_exec( memcopy_gpu2host(d_fkm1, h_fkm+i_step*2*nwfip*NXY,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
...
//             if(ip==0) { wprintf("# SELFSTART: i_step=%d\n", i_step); fflush(stdout); }
...
#if INTEGRATION_SCHEME==AB3AM4
        gpu_exec( memcopy_host2gpu(h_fkm+2*2*nwfip*NXY, d_fkm1,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+1*2*nwfip*NXY, d_fkm2,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXY, d_fkm3,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
#elif INTEGRATION_SCHEME==AB4AM5
        gpu_exec( memcopy_host2gpu(h_fkm+3*2*nwfip*NXY, d_fkm1,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+2*2*nwfip*NXY, d_fkm2,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+1*2*nwfip*NXY, d_fkm3,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXY, d_fkm4,  (size_t)2*nwfip*NXY*sizeof(double complex)) ); 
#elif INTEGRATION_SCHEME==AB5AM5
        gpu_exec( memcopy_host2gpu(h_fkm+4*2*nwfip*NXY, d_fkm1,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+3*2*nwfip*NXY, d_fkm2,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+2*2*nwfip*NXY, d_fkm3,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+1*2*nwfip*NXY, d_fkm4,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXY, d_fkm5,  (size_t)2*nwfip*NXY*sizeof(double complex)) );
#endif        
    ...
    for (i_meas=0; i_meas<md.measurements; i_meas++)
    {
        ...
        for(i_step=0; i_step<md.timesteps; i_step++)
        {
            ...
            {
                gpu_exec( calculate_quantum_friction_densities(nwfip, d_wf, d_wf_laplace, d_kkz, d_fbetaEn, NULL, d_qf_densities, md.nthreads) );
                mpipackagesize = 12+4;
            }
            ...
            gpu_exec( compute_laplace(2*nwfip, d_alphawf_laplace, d_alphawf_laplace, md.nthreads) );
#endif
            if(qfswitch_it>0.0) pccoeff=quantum_friction_pccoeff(NXY,densall.rho_a,densall.rho_b,DX*DY*LZ); // see pca_utils.c
            // H*psi
