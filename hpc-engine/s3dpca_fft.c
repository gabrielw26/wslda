...
int high_frequency_filter_c(double complex *in, double complex *out, double fd_mu, double fd_T, metadata_s3dpca_fft *mdfft)
{
    int ixyz;
    int ix, iy, iz;
    double kx, ky, kz, ek;
    ...
    // copy data to working array
    for(ixyz=0; ixyz<NXYZ; ixyz++) mdfft->fft3[ixyz] = in[ixyz];
    ...
    // copy data to result table
    for(ixyz=0; ixyz<NXYZ; ixyz++) out[ixyz]=mdfft->fft3[ixyz];
