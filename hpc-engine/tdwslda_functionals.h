...
__global__ void tdwslda_compute_potentials(int it, wslda_density h_densities, wslda_potential h_potentials, double cccoeff)
{
    ...
    if(ixyz<NUMBER_ELEMENT)
    {
        ...
        if(t7!=0.0)
        {
            ...
            Vb_const+=( (alph_plus-alph_minus-1.0)/nb - (dalphp_dnb-dalphm_dnb) ) *t7; // fr(nb)*(alpha_b-1)*jb^2/2nb^2 - fr(nb)*dalphb_dnb*jb^2/2nb
...
__global__ void tdwslda_compute_potentials(int it, wslda_density h_densities, wslda_potential h_potentials, double cccoeff)
{
    ...
    Complex p0, kc, wz_0; // spherical cutoff integral
    Complex lnu, ldelta;
    ...
    if(ixyz<NUMBER_ELEMENT)
    {
        ...
        if(t7!=0.0)
        {
            ...
            Vb_const+=( (af_-1.0)/nb - af_p ) *t7; // fr(nb)*(alpha_b-1)*jb^2/2nb^2 - fr(nb)*dalphb_dnb*jb^2/2nb
        ...
        for(i=0; i<UD_SCITERS; i++) // self-consistent loop
        {
          ...
          p0 = thrust::sqrt( Complex(2.0*lmu_sc/ af_, 0.0) );
          if(p0.imag()<0.) p0 *= -1. ;
          #ifdef USE_CUBIC_CUTOFF
          kc=M_PI/DX;
          #else
          kc = thrust::sqrt( Complex(2.0*(dc_ec+lmu_sc)/ af_, 0.0) );
          if(kc.imag()<0.) kc *= -1. ;
          #endif
