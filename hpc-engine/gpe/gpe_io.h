...
#define mio_hentry_read(var,type,werrval) \
    ...
    if(werr==0 && prec!=PRECSION) werr=-3;
...
int touch_file(const char * file_name)
{
    if(exists(file_name)) 
    {
        if(md_overwrite==0) return -1; // We do not overwrite!  
        else urm(file_name);
    }
...
int append_to_binary_file(const char * file_name, unsigned long int size, void * data)
{     
    int fd ; /* file descriptor for handling the file or device */
    long int bytes_rw ;
