...
extern "C" int create_cufftPlans(int batch_size,  int nwfip, size_t *workSize)
{
    ...
    *workSize = 0;
    ...
    // Plans of type ONE reserve extra space for working areas
    ...
    if(cufft_result!= CUFFT_SUCCESS) return (int)cufft_result;
    max_work_Size+=mm_get_size_of_total_workspace(NXY);
    if(max_work_Size>*workSize) *workSize=max_work_Size; 
...
extern "C" int set_workspace_for_cufftPlan(void *workArea)
{
    ...
    cufftDoubleComplex *prt = (cufftDoubleComplex *)mm_get_pointer_to_fftONE_workspace(NXY);
...
extern "C" int compute_gradient_real_f(double *f, double *df_dx, double *df_dy, double *df_dz, int nthreads)
{
    ...
    cufftDoubleComplex * p_df_dx = (cufftDoubleComplex *)mm_get_pointer_to_derivatives_workspace(NXY);
                                                ...
                                                int nthreads)
{
    ...
    // get pointer to workspace
    cufftDoubleComplex * p_fx = (cufftDoubleComplex *)mm_get_pointer_to_derivatives_workspace(NXY);
