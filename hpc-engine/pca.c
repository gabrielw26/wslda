...
int main( int argc , char ** argv ) 
{
    ...
    size_t  workarea_size=(size_t)mm_get_size_of_total_workspace(NXYZ); // minimal size of workarea
    int mpipackagesize;
...
//     if(ip==0 && md.spinsymmetry==1) print_warning(WSLDA_WRN_SPINSYMMETRY1);
    ...
    double *d_qf_densities=d_densities + 12*NXYZ; // to simplify notation
...
//         wprintf("# WF SCATTER: ip=%d processes nwfip=%d wave-functions\n", ip, nwfip);
    ...
    else if(md.inittype==3) // Start from st-wslda-3d solution
    {
        ...
        time=0.0;
        b_t();
        file_operation( check_comptibility(md.inprefix, 3, file_suffix) );
        sprintf(file_name, "%s/%s.info", md.inprefix, file_suffix);
        if(ip==0)
        {
            file_operation( read_checkpoint_info_pca(file_name, &nwf, &_nx, &_ny, &_nz, &_dx, &_dy, &_dz, &kF, &mu[0], &ec, &beta) );
...
//     if(md.inittype!=5) gpu_exec( compute_potentials(it, d_densities, d_potentials, 0.0, md.nthreads) );
    ...
    { 
        ...
        for(i_step=0; i_step<selfstart_steps; i_step++)
        {
            ...
            for(i_meas=0; i_meas<selfstart_steps-1; i_meas++) memcpy(h_fkm+(i_meas+1)*2*nwfip*NXYZ, h_fkm+i_meas*2*nwfip*NXYZ, (size_t)2*nwfip*NXYZ*sizeof(double complex));
            // Store H*Psi
            gpu_exec( memcopy_gpu2host(d_fkm1, h_fkm,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
...
//             if(ip==0) { wprintf("# SELFSTART: i_step=%d\n", i_step); fflush(stdout); }
        ...
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXYZ, d_fkm1,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
#elif INTEGRATION_SCHEME==AB4AM5
        gpu_exec( memcopy_host2gpu(h_fkm+3*2*nwfip*NXYZ, d_fkm4,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+2*2*nwfip*NXYZ, d_fkm3,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+1*2*nwfip*NXYZ, d_fkm2,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXYZ, d_fkm1,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) ); 
#elif INTEGRATION_SCHEME==AB5AM5
        gpu_exec( memcopy_host2gpu(h_fkm+4*2*nwfip*NXYZ, d_fkm5,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+3*2*nwfip*NXYZ, d_fkm4,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+2*2*nwfip*NXYZ, d_fkm3,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+1*2*nwfip*NXYZ, d_fkm2,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) );
        gpu_exec( memcopy_host2gpu(h_fkm+0*2*nwfip*NXYZ, d_fkm1,  (size_t)2*nwfip*NXYZ*sizeof(double complex)) );
    ...
    for (i_meas=0; i_meas<md.measurements; i_meas++)
    {
        ...
        for(i_step=0; i_step<md.timesteps; i_step++)
        {
            ...
            gpu_exec( compute_laplace(2*nwfip, d_alphawf_laplace, d_alphawf_laplace, md.nthreads) );
#endif
            if(qfswitch_it>0.0) pccoeff=quantum_friction_pccoeff(NXYZ,densall.rho_a,densall.rho_b,DX*DY*DZ); // see pca_utils.c
            // H*psi
