...
int main( int argc , char ** argv )
{
    ...
    wslda_potential potsall_old = convert_into_wslda_potential(h_potentials_old, BLOCKLENGTH, mu);
        ...
        if(md.spinsymmetry==0 && elpa_nev<Hsize && En[elpa_nev-1]<dc_ec)
        {
            ...
            if(pzheevr_m==0)
            {
                ...
                if(saving_iteration==1 && gr_iam==0)
                {
                    ...
                    sprintf(file_name, "%s/wf.%04d_vn.wdat", md.outprefix, ikz);
                    file_operation( touch_file(file_name) );
                    ...
                    sprintf(file_name, "%s/wf.%04d_en.dat", md.outprefix, ikz);
                    file_operation( touch_file(file_name) );
                    sprintf(file_name, "%s/wf.%04d_en.txt", md.outprefix, ikz);
                    file_operation( touch_file(file_name) );
                }
...
// #endif
            ...
            if(md.spinsymmetry==1) convert_eigenstates_negative_into_positive(niq_d, BLOCKLENGTH, En_d_local, U_d);
#endif
            rt_redistrib+=e_t(0);
        ...
        b_t();
        double Rnorm_V_a=wslda_residual_norm_d(BLOCKLENGTH, potsall.V_a, potsall_old.V_a)/eF;
        double Rnorm_V_b=wslda_residual_norm_d(BLOCKLENGTH, potsall.V_b, potsall_old.V_b)/eF;
        double Rnorm_delta=wslda_residual_norm_c(BLOCKLENGTH, potsall.delta, potsall_old.delta)/eF;
