...
// Padé[4/4] fitting function (unitary constraint)
// ===============================================
// \partial_n pade_4_4
// = \partial_n (a1*x + a2*x^2 + a3*x^3 + c*x^4)/(1 + b2*x^2 + b3*x^3 + c*x^4)
FDECORATOR double
pade_4_4 (int dn, double x, double a1, double a2, double a3, double b1, double b2, double b3, double c)
{
  if (dn <= FUNCTIONAL_ORDER) {
    if (dn == 0) {
      return (a1 * x + a2 * pow(x, 2) + a3 * pow(x, 3) + c * pow(x, 4))
           / (1 + b1 * x + b2 * pow(x, 2) + b3 * pow(x, 3) + c * pow(x, 4));
    } else if (dn == 1) {
      return -(((b1 + 2.*b2*x + 3.*b3*pow(x,2) + 4.*c*pow(x,3))* (a1*x + a2*pow(x,2) + a3*pow(x,3) + c*pow(x,4)))/ pow(1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4),2)) + (a1 + 2.*a2*x + 3.*a3*pow(x,2) + 4.*c*pow(x,3))/ (1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4));
    } else if (dn == 2) {
      return (-2.*(a1 + 2.*a2*x + 3.*a3*pow(x,2) + 4.*c*pow(x,3))* (b1 + 2.*b2*x + 3.*b3*pow(x,2) + 4.*c*pow(x,3)))/ pow(1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4),2) + (2.*a2 + 6.*a3*x + 12.*c*pow(x,2))/ (1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4)) + (a1*x + a2*pow(x,2) + a3*pow(x,3) + c*pow(x,4))* ((2.*pow(b1 + 2.*b2*x + 3.*b3*pow(x,2) + 4.*c*pow(x,3),2))/ pow(1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4),3) - (2.*b2 + 6.*b3*x + 12.*c*pow(x,2))/ pow(1. + b1*x + b2*pow(x,2) + b3*pow(x,3) + c*pow(x,4),2));
    } else {
      return 0.;
    }
  } else {
...
inverse_effective_mass (int dn, double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    a1 = 0.;
    a2 = U_APS * (A_APS + B_APS * U_APS);
    a3 = U_APS * (2. * A_APS * V_APS + B_APS * U_APS * V_APS);
    a4 = A_APS * U_APS * (pow(U_APS, 2) + pow(V_APS, 2));
    b1 = 4. * V_APS;
    b2 = (2. * pow(U_APS, 2) + 6. * pow(V_APS, 2));
    b3 = 4. * V_APS * (pow(U_APS, 2) + pow(V_APS, 2));
    b4 = pow(pow(U_APS, 2) + pow(V_APS, 2), 2);
    if (dn == 0) {
      return 1. + (a1 * _x + a2 * pow(_x, 2) + a3 * pow(_x, 3) + a4 * pow(_x, 4))
            / (1 + b1 * _x + b2 * pow(_x, 2) + b3 * pow(_x, 3) + b4 * pow(_x, 4));
    } else if (dn == 1) {
...
beta_parameter (int dn, double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    if (dn <= FUNCTIONAL_ORDER) {
      ...
      return BF_APS_UFG * pade_4_4(dn, _x, a1, a2, a3, b1, b2, b3, c);
    } else {
      return 0.;
    }
  } else {
...
inverse_gamma_parameter (int dn, double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    if (dn <= FUNCTIONAL_ORDER) {
      if (dn == 0) {
        return (1. / CF_APS_UFG) / pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c);
      } else if (dn == 1) {
        return (1. / CF_APS_UFG)
          * pade_4_4(1, _x, a1, a2, a3, b1, b2, b3, c)
          / pow(pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c), 2);
      } else if (dn == 2) {
        return (1. / CF_APS_UFG)
          * (pade_4_4(2, _x, a1, a2, a3, b1, b2, b3, c)
            * pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c)
            - pow(pade_4_4(1, _x, a1, a2, a3, b1, b2, b3, c), 2))
          / pow(pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c), 4);
      } else {
        return 0.;
      }
    } else {
...
a_functional (double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    r_ = 0.;
    r_ += alpha_parameter(0, _x, id);
    r_ += (-_x) * alpha_parameter (1, _x, id)
                * tgamma (d_ + 1.) / tgamma (d_ + 1. + 1.);
    r_ += pow(-_x, 2) * alpha_parameter (2, _x, id)
                * tgamma (d_ + 1.) / tgamma (d_ + 1. + 2.);
    return r_;
  } else {
    return 0.;
  }
...
b_functional (double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    return r_;
  } else if (fid == 0) { // fit with Padé[4/4]
    double a1, a2, a3, a4, b1, b2, b3, b4, c;
    //Final set of parameters            Asymptotic Standard Error
    //=======================            ==========================
    ...
    return BF_APS_UFG * pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c);
  } else {
    return 0.;
  }
}
...
c_functional (double _x, int id [])
{
  ...
  if (fid == -2) {
    ...
    return CF_APS_UFG * pade_4_4(0, _x, a1, a2, a3, b1, b2, b3, c);
  } else {
    return 0.;
  }
}
