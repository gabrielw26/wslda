...
extern "C" int create_cufftPlans(int batch_size,  int nwfip, size_t *workSize)
{
    ...
    *workSize = 0;
    ...
    if(cufft_result!= CUFFT_SUCCESS) return (int)cufft_result;
    max_work_Size+=mm_get_size_of_total_workspace(NXYZ);
    if(max_work_Size>*workSize) *workSize=max_work_Size; 
...
extern "C" int set_workspace_for_cufftPlan(void *workArea)
{
    ...
    cufftDoubleComplex *prt = (cufftDoubleComplex *)mm_get_pointer_to_fftONE_workspace(NXYZ);
...
extern "C" int compute_gradient_real_f(double *f, double *df_dx, double *df_dy, double *df_dz, int nthreads)
{
    ...
    cufftDoubleComplex * p_df_dx = (cufftDoubleComplex *)mm_get_pointer_to_derivatives_workspace(NXYZ);
                                                ...
                                                int nthreads)
{
    ...
    // get pointer to workspace
    cufftDoubleComplex * p_fx = (cufftDoubleComplex *)mm_get_pointer_to_derivatives_workspace(NXYZ);
...
#ifdef DERIVATIVE_COPY_DATA_MODE
    double * p_tmp = (double *)(p_fx+NX*NY*(NZ/2+1));
    if( cudaMemcpy( p_tmp , in , NXYZ*sizeof(double), cudaMemcpyDeviceToDevice )!= cudaSuccess ) return -333;
    cufft_result=cufftExecD2Z(__md_pca_cufftplans.plans[PLAN_D2Z_ONE], p_tmp, p_fx);
#else
                                     ...
                                     int nthreads)
{
    ...
    // Step 1: go to momentum space
#ifdef DERIVATIVE_COPY_DATA_MODE
    cufftDoubleComplex * p_tmp = (cufftDoubleComplex *)(p_fx+NXYZ);
    if( cudaMemcpy( p_tmp , in , NXYZ*sizeof(cufftDoubleComplex), cudaMemcpyDeviceToDevice )!= cudaSuccess ) return -333;
