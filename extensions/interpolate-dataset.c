 ...
 * W-SLDA Toolkit
 * 
 * This tool interpolates w-data set to new lattice. 
...
int main( int argc , char ** argv ) 
{
    ...
    for(ivar=0; ivar<wdmdo.nvar; ivar++)
    {
        ...
        if (strcmp (wdmdo.var[ivar].type,"real") == 0)
        {
            for(icycle=0; icycle<wdmdo.cycles; icycle++) // for each cycle
            {
                ...
                if     (wdmdo.datadim==3) wslda_interpolation_3dr(inNX, inNY, inNZ, indata, NX, NY, NZ, outdata);
                else if(wdmdo.datadim==2) wslda_interpolation_2dr(inNX, inNY,       indata, NX, NY,     outdata);
                else                      wslda_interpolation_1dr(inNX,             indata, NX,         outdata);
                file_operationl( wdata_write_cycle(&wdmdo, wdmdo.var[ivar].name, outdata) );
            }
        }
        
        if (strcmp (wdmdo.var[ivar].type,"complex") == 0)
        {
            for(icycle=0; icycle<wdmdo.cycles; icycle++) // for each cycle
            {
                file_operationl( wdata_read_cycle(&wdmd, wdmdo.var[ivar].name, icycle, indata) );
                if     (wdmdo.datadim==3) wslda_interpolation_3dc(inNX, inNY, inNZ, (double complex *)indata, NX, NY, NZ, (double complex *)outdata);
                else if(wdmdo.datadim==2) wslda_interpolation_2dc(inNX, inNY,       (double complex *)indata, NX, NY,     (double complex *)outdata);
                else                      wslda_interpolation_1dc(inNX,             (double complex *)indata, NX,         (double complex *)outdata);
                file_operationl( wdata_write_cycle(&wdmdo, wdmdo.var[ivar].name, outdata) );
            }
        }
        
        if (strcmp (wdmdo.var[ivar].type,"vector") == 0)
        {
            for(icycle=0; icycle<wdmdo.cycles; icycle++) // for each cycle
            {
                file_operationl( wdata_read_cycle(&wdmd, wdmdo.var[ivar].name, icycle, indata) );
                if(wdmdo.datadim==3)
                {
                    wslda_interpolation_3dr(inNX, inNY, inNZ, indata+0*inNX*inNY*inNZ, NX, NY, NZ, outdata+0*NX*NY*NZ);
                    wslda_interpolation_3dr(inNX, inNY, inNZ, indata+1*inNX*inNY*inNZ, NX, NY, NZ, outdata+1*NX*NY*NZ);
                    wslda_interpolation_3dr(inNX, inNY, inNZ, indata+2*inNX*inNY*inNZ, NX, NY, NZ, outdata+2*NX*NY*NZ);
                }
                else if(wdmdo.datadim==2)
                {
                    wslda_interpolation_2dr(inNX, inNY, indata+0*inNX*inNY, NX, NY, outdata+0*NX*NY);
                    wslda_interpolation_2dr(inNX, inNY, indata+1*inNX*inNY, NX, NY, outdata+1*NX*NY);
                    wslda_interpolation_2dr(inNX, inNY, indata+2*inNX*inNY, NX, NY, outdata+2*NX*NY);
                }
                else
                {
                    wslda_interpolation_1dr(inNX, indata+0*inNX, NX, outdata+0*NX);
                    wslda_interpolation_1dr(inNX, indata+1*inNX, NX, outdata+1*NX);
                    wslda_interpolation_1dr(inNX, indata+2*inNX, NX, outdata+2*NX);
                }
                file_operationl( wdata_write_cycle(&wdmdo, wdmdo.var[ivar].name, outdata) );
            }
        }
