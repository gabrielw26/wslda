...
int main( int argc , char ** argv )
{
    ...
    double aSLDAe;
#if FUNCTIONAL==SLDAE
    aSLDAe = input->aSLDAe; // copy to global momeory
    if ( fabs(aSLDAe)<1.0e-12 )
    {
      printf("ERROR: SET aSLDAe TO A NON-ZERO VALUE IN INPUT FILE!\n");
      return( EXIT_FAILURE ) ;
    }
    #else
    aSLDAe = 0.0; // deactivate SLDAE functional
#endif
...
#elif FUNCTIONAL==SLDAE
    if(fabs(aSLDAe)<1.0e-12) solve_uniform_problem    (input->init0Na/LXYZ, input->init0Nb/LXYZ, &nwf, 1);
    else                   solve_uniform_problem_sldae(input->init0Na/LXYZ, input->init0Nb/LXYZ, &nwf, 1);
#else
