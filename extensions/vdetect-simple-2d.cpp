...
extern "C" double_complex interpolate(double_complex *fcoeffs, double x, double y, int nx, int ny);
extern "C" int check_if_monotonic_down(const int size, double * args);
extern "C" double pdiff(double arg1, double arg2);
extern "C" double my_f (const gsl_vector *v, void *params);
...
int main( int argc , char* argv[] )
{
    ...
    double_complex *delta_k;
    cppmallocl(delta_k,nx*ny,double_complex);
    fftw_plan plan_f = fftw_plan_dft_2d(nx, ny, reinterpret_cast<fftw_complex*>(delta_k), reinterpret_cast<fftw_complex*>(delta_k), FFTW_FORWARD, USE_FFTW_PLANNER);
    fftw_plan plan_b = fftw_plan_dft_2d(nx, ny, reinterpret_cast<fftw_complex*>(delta_k), reinterpret_cast<fftw_complex*>(delta_k), FFTW_BACKWARD, USE_FFTW_PLANNER);
        
    double_complex *rho_a_k, *rho_b_k;
    cppmallocl(rho_a_k,nx*ny,double_complex);
    cppmallocl(rho_b_k,nx*ny,double_complex);
    
    // order parameter
    double_complex *delta;
    cppmallocl(delta, nxyz, double_complex);
    ...
    // j = [jx(r),jy(r),jz(r)]
    double *ja_x=j_a;
    double *ja_y=j_a+nxyz;
    double *jb_x=j_b;
    double *jb_y=j_b+nxyz;    
...
//         printf("# inom=%d...\n", inom);
        ...
        wdata_read_cycle(&md, "j_a", inom, j_a);
        wdata_read_cycle(&md, "j_b", inom, j_b);      
        ...
        fftw_execute(plan_f);  
        for(ixyz=0; ixyz<nxyz; ixyz++) rho_a_k[ixyz]=delta_k[ixyz]/(1.0*nxyz); 
        
        for(ixyz=0; ixyz<nxyz; ixyz++) delta_k[ixyz]=rho_b[ixyz];
        fftw_execute(plan_f);  
        for(ixyz=0; ixyz<nxyz; ixyz++) rho_b_k[ixyz]=delta_k[ixyz]/(1.0*nxyz); 
        ...
        fftw_execute(plan_f);  
        for(ixyz=0; ixyz<nxyz; ixyz++) delta_k[ixyz]=delta_k[ixyz]/(1.0*nxyz);     
...
//             }
// 
//             printf ("%5d %12.9f %12.9f f() = %12.9f size = %.4f\n",
//                     iter,
...
//                     s->fval, size);
        ...
        // no shift
        na[0]=(interpolate(rho_a_k, _xs, _ys, nx, ny)).real();
        nb[0]=(interpolate(rho_b_k, _xs, _ys, nx, ny)).real();
