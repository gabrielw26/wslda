 ...
 * 
 * Tool for sampling subset from wdata set
 * */
...
{
    printf("WDATA SAMPLER:\n\t creates new data set, where only every `stride` measurment is present\n");
    ...
    if(argc!=4)
    {
        ...
        printf("\tstride       - take only every stride measurment\n");
        return 0;
    }
    ...
    int icycle, ivar;
    for(icycle=0; icycle<md.cycles; icycle+=stride)
    {
