...
int main( int argc , char ** argv ) 
{
    ...
    if(argc!=5 && argc!=6)
    {
        ...
        printf("\tor\n");
        printf("Usage: %s file.wtxt outprefix start stop stride\n", argv[0]);
        ...
        printf("\tstride       - every `stride` frame will be taken only from given range, default stride=1\n");
        return 0;
    }
    ...
    int icycle, ivar;
    for(icycle=0; icycle<md.cycles; icycle++) if(icycle>=start && icycle<stop && icycle%stride==0)
    {
    ...
    fprintf(f,"# Generated by: %s %s %s %s %s %d\n\n", argv[0], argv[1], argv[2], argv[3], argv[4], stride);
