           ...
           double *params, size_t extra_data_size, void *extra_data)
{
    ...
    if(params[12]>0.0) errRho=params[12];
    if(params[13]>0.0) errDelta=params[13];
    ...
    if(errN>0.0) 
    {
        ...
        for(ix=0; ix<NX; ix++)
        {
            fprintf(fcmp,"rho[%d]     %20.10g %20.10g\n", ix, h_densities.rho_a[ix], errRho);
            fprintf(fcmp,"delta[%d]   %20.10g %20.10g\n", ix, cabs(h_potentials.delta[ix]), errDelta);
        }
    }
    else
    {
        ...
        fprintf(fcmp,"energy[ETOT]     %20.10g\n", E_tot);
        for(ix=0; ix<NX; ix++)
        {
            fprintf(fcmp,"rho[%d]     %20.10g\n", ix, h_densities.rho_a[ix]);
            fprintf(fcmp,"delta[%d]   %20.10g\n", ix, cabs(h_potentials.delta[ix]));
        }
    }    
