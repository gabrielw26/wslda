...
void process_params(double *params, double *kF, double *mu, size_t extra_data_size, void *extra_data)
{
    ...
    params[1]*=(2.*M_PI/LX) / 2.0; // factor 2 accounts for mass of Cooper pair
}
...
//     double z = DZ*(iz-NZ/2);     // for 1d and 2d codes iz will be always 0
    ...
    double v_ext, vx=0.0;
    if(params[0]>1.99) vx=params[1]; // methods 2 and 3
    if(coordinate==XAXIS) v_ext=vx;
...
{    
    ...
    double grad=0.0;
    if(params[0]>0.99 && params[0]<1.01) grad=params[1]*2.0;  // method 1
    
    // ITERATE OVER ALL POINTS
    ixyz=0;
    if(it<1) for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++) // initilization
    {
        double x = DX*(ix-lNX/2);
        double y = DY*(iy-lNY/2); // for 1d code y will be always 0
        double z = DZ*(iz-lNZ/2); // for 1d and 2d codes z will be always 0

        double delta_abs=cabs(h_potentials.delta[ixyz]);
        h_potentials.delta[ixyz] = cexp(I*params[1]*2.0*x)*delta_abs;

        ixyz++; // go to the next point, it should be the last line of the triple loop
    }

    // target phase imprint
    ixyz=0;
    for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++)
    {
        double x = DX*(ix-lNX/2);
        ...
        double delta_abs=cabs(h_potentials.delta[ixyz]);
        if(params[0]>0.99 && params[0]<2.01) // do phase imprint if method=1,2
            h_potentials.delta[ixyz] = cexp(I*grad*x)*delta_abs;
