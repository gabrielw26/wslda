...
{
    return 0.5*M_PI*((input->Na+input->Nb)/LX);
}
...
{    
    ...
    if(it==-1) for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++)
    {
        ...
        double kF = 0.5*M_PI*((input->Na+input->Nb)/LX);
        double eF = 0.5*kF*kF;

        h_densities.rho_a[ixyz]=input->Na/LX*10; // guess for the density, here I set constant value
        h_densities.rho_b[ixyz]=input->Nb/LX*10; // guess for the density, here I set constant value

        // all potentials must be filled, since they are used to construct BdG matrix
        h_potentials.delta[ixyz]=0.1*eF + I*0.0; // guess for delta
    ...
    if(it==-1)
    {
        double kF = 0.5*M_PI*((input->Na+input->Nb)/LX);
        double eF = 0.5*kF*kF;
        h_potentials.mu[SPINA]=0.9*eF;
        h_potentials.mu[SPINB]=0.9*eF;
    }
    ...
    if(it!=-1) for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++)
    {
        ...
        // ... below you can modify them at your wish ...
        h_potentials.delta[ixyz] = -1.0*h_densities.nu[ixyz]*params[1]; // custom gap equation

        // if(it<100 && cabs(h_potentials.delta[ixyz])<0.05) h_potentials.delta[ixyz]=0.05;
