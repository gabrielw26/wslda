...
//     double z = DZ*(iz-NZ/2);     // for 1d and 2d codes iz will be always 0
    ...
    double V_ext = 0.5*pow(x*params[1],2); // harmonic oscilator
...
{    
    ...
    if(it==-1) for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++)
    {
        ...
        h_potentials.delta[ixyz]=0.0 + I*0.0; // guess for delta
    ...
    if(it==-1)
    {
        h_potentials.mu[SPINA]=params[2];
        h_potentials.mu[SPINB]=params[3];
    }
    
    ixyz=0;
    for(ix=0; ix<lNX; ix++) for(iy=0; iy<lNY; iy++) for(iz=0; iz<lNZ; iz++)
    {
        h_potentials.delta[ixyz]=0.0 + I*0.0; // no pairing, HF-mode
        ixyz++; // go to next point,  it should be last line of the triple loop
    }
}
