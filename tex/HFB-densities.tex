...
\subsection{For any temperature $T \ge 0$}
...
For $T = 0$ (and therefore $\beta \to \infty$) the value of $f_\beta (\pm E)$ sets to fixed values:

\begin{eqnarray}
$f_{\beta}(+E) \xrightarrow[\beta \to \infty]{} 1/(\exp(+E * \infty)+1) = 0$, \\
&& $f_{\beta}(-E) \xrightarrow[\beta \to \infty]{} 1/(\exp(-E * \infty)+1) = 1$.
\label{eqn:distributions}
\end{eqnarray}

$f_{\beta}(+E)$ converges to $0$ from the positive numbers' side, resulting in $f_{\beta}(+E)\ln {f_{\beta}(+E)} \xrightarrow[\beta \to \infty]{} 0$. The equations of densities change to:

\begin{eqnarray}
n_{\uparrow}(\bm{r})=n_{\downarrow}(\bm{r}) &=& \sum_{0<E_n<E_c}\abs{v_{n}(\bm{r})}^2\\
\tau_{\uparrow}(\bm{r})=\tau_{\downarrow}(\bm{r}) &=& \sum_{0<E_n<E_c}\abs{\nabla v_{n}(\bm{r})}^2,\\
\nu(\bm{r}) &=& \sum_{0<E_n<E_c} u_{n}(\bm{r})v_{n}^{*}(\bm{r}),\\
\bm{j}_{\uparrow}(\bm{r})=\bm{j}_{\downarrow}(\bm{r}) &=& \sum_{0<E_n<E_c}\textrm{Im}[v_{n}(\bm{r})\nabla v_{n}^*(\bm{r})] f_{\beta}(-E_n),\\
S&=&0.
\label{eqn:densities3}
\end{eqnarray}
